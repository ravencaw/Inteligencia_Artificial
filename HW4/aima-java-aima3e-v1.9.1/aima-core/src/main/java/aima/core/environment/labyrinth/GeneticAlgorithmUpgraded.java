 ...
 */
public class GeneticAlgorithmUpgraded<A> {
	...
	public GeneticAlgorithmUpgraded(int individualLength, Collection<A> finiteAlphabet, double mutationProbability) {
	...
	protected List<Individual<A>> nextGeneration(List<Individual<A>> population, FitnessFunction<A> fitnessFn) {
		...
		List<Individual<A>> newPopulation = new ArrayList<Individual<A>>(population.size());
                // MRP: We add the best individual of the previous generation to the new one before reproducing the rest
                Individual<A> prevBest = retrieveBestIndividual(population, fitnessFn);
                newPopulation.add(prevBest);
                //population.remove(prevBest); // We want to use the fittest invividual to reproduce (hence the -1 in the loop)
		// for i = 1 to SIZE(population) do
	...
	protected Individual<A> reproduce(Individual<A> x, Individual<A> y) {
		...
		// c <- random number from 1 to n, first cut
                // d <- random number from 1 to n, second cut
		int c = randomOffset(individualLength);
		int d = randomOffset(individualLength);
		// return APPEND(SUBSTRING(x, 1, c & d+1, n), SUBSTRING(y, c+1, d))
		List<A> childRepresentation = new ArrayList<A>();
                if (c<d) {
                    ...
                    childRepresentation.addAll(x.getRepresentation().subList(0, d));
                    childRepresentation.addAll(y.getRepresentation().subList(d, c));
	...
	protected Individual<A> mutate(Individual<A> child) {
		...
		mutatedRepresentation.set(mutateOffset1, finiteAlphabet.get(alphaOffset));
		mutatedRepresentation.set(mutateOffset2, finiteAlphabet.get(alphaOffset));
